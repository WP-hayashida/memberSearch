...
const headerColmunArr =[
    ...
    {'id':'gradeSub','name':'SPグレード','sort':true,'sortCol':'MEGRADE_sub','sortAorD':1,'header':2,'rowspan':1,'width':'10%','csvNo':13,'data':'MEGRADE_sub'},
    {'id':'officeAccg','name':'拠点(会計)','sort':true,'sortCol':'MEOFFICE_ACC','sortAorD':0,'header':2,'rowspan':1,'width':'10%','csvNo':15,'data':'name_MEOFFICE_ACC'}
];
...
function exportCsvAction(){
    exportCsv('accountingSearch', resultCSVArr);
}
...
function createResultCSV(resultArr, Ym) {
    ...
    const resultCSVArr = resultArr.map(item => {
        ...
        return {
            ...
            会計拠点コード: item.MEOFFICE_ACC,
            会計拠点名: item.name_MEOFFICE_ACC,
            対象年月: Ym,
            出力日: outputDate
        };
    });
...
function search(sortId) {
    ...
    setupPagination(resultArr, itemsPerPage);
}
...
function setupPagination(resultArr, itemsPerPage) {
    ...
    $('#pagination, #paginationBottom').twbsPagination({
        ...
        onPageClick: function (event, page) {
            ...
            const endIndex = Math.min(Number(startIndex) + Number(itemsPerPage), resultArr.length);
            const pageItems = resultArr.slice(startIndex, endIndex);
            console.log(itemsPerPage,endIndex)
