...
///////////////////////////////
        ...
        FROM (
            SELECT
                ...
                MEGRADE_sub,
                CASE
                    WHEN TM_WPHEADQUARTER.pic = TM_MEMBER_archive.MEUSERID THEN 'head-pic'
                    WHEN TM_WPDEPARTMENT.pic = TM_MEMBER_archive.MEUSERID THEN 'depart-pic'
                    WHEN M.manager = TM_MEMBER_archive.MEUSERID THEN 'manager'
            ...
            LEFT JOIN TM_WPHEADQUARTER ON TM_WPHEADQUARTER.pic = TM_MEMBER_archive.MEUSERID
            LEFT JOIN TM_WPDEPARTMENT ON TM_WPDEPARTMENT.pic = TM_MEMBER_archive.MEUSERID
            LEFT JOIN TM_WPUNIT_archive AS M ON M.manager = TM_MEMBER_archive.MEUSERID AND M.invalid = 0 AND U.date = ?
            LEFT JOIN TM_WPTEAM_archive ON TM_WPTEAM_archive.teamleader = TM_MEMBER_archive.MEUSERID AND TM_WPTEAM_archive.invalid = 0 AND T.date = ?
            WHERE TM_MEMBER_archive.date = ?
			AND LEFT(MECD,2) <> '00'
            AND RIGHT(MEUSERID,4) <> '_ptn'
            AND MEINVALIDFLAG = 0
        ) AS sub
    )
    SELECT * FROM MemberWithRowNum WHERE rn = 1
